...
class SKAttention(nn.Module):
    def __init__(self, c1, c2, reduction=16) -> None:
        super().__init__()

        # Standard convolution with args(ch_in, ch_out, kernel, stride, padding, groups, dilation, activation)
        self.cvu1 = Conv(c1, c1, k=3, p=3 // 2, act=nn.ReLU(inplace=True))

        self.cvu2 = Conv(c1, c1, k=5, p=5 // 2, act=nn.ReLU(inplace=True))
        d = max(32, c1 // reduction)

        self.global_pooling = nn.AdaptiveAvgPool2d(1)

        self.fc = nn.Sequential(
            nn.Linear(c1, d), nn.ReLU(inplace=True), nn.Linear(d, c1), nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        s = self.global_pooling(U1 + U2).squeeze()

        z = self.fc(s).view(bs, c, 1, 1)
